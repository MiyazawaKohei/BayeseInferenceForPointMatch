...
vector<double> Fix_Gravity_Point_to_Origin(vector<double*> vectors){
	vector<double*> newvectors;
	vector<double> gravity_point(3);
...
double Maximize_trRBAt(const vector<double*> A,const vector<double*> B,Eigen::Matrix<double,3,3>* R){
...
double Minimize_RMSD(const vector<double*> A,const vector<double*> B, const vector<int> sigma_i,Eigen::Matrix<double,3,3>* R,vector<double>* v){
	...
	vector<double> offsetA=Fix_Gravity_Point_to_Origin(subA);
	vector<double> offsetB=Fix_Gravity_Point_to_Origin(subB);
	
	double RMSD=0;
	for (int i=0; i<sigma_i.size(); i++){
		for(int j=0; j<dimention; j++) RMSD+=subA[i][j]*subA[i][j]+subB[i][j]*subB[i][j];
	}
	RMSD-=2*Maximize_trRBAt(subA,subB,R);
...
void Make_Distance_Matrix(vector<double*>* Dmatrix,const vector<double*> A,const vector<double*> B,Eigen::Matrix<double,3,3>* R,vector<double>* v){
	double variance;
	for(int i=0; i<A.size(); i++){
		for(int j=0; j<B.size(); j++){
			...
			for(int k=0; k<3; k++){
				variance=A[i][k]-(*v)[k];
				for(int l=0; l<3; l++){
					variance-=(*R)(k,l)*B[j][l];	
				}
				(*Dmatrix)[i][j]+=variance*variance;
			}
		}
	}
	return;
